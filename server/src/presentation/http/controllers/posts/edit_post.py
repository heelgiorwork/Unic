...
from src.services.commands.edit_post import (
    EditPostInteractor,
    EditPostRequest,
)
...
def create_edit_post_router() -> APIRouter:
    router = ErrorAwareRouter()

    @router.patch(
        "/{post_id}",
        description=getdoc(EditPostInteractor),
        error_map={
    ...
    async def edit_post(
        ...
        interactor: FromDishka[EditPostInteractor],
    ) -> None:
